...
from voice_alert_system import generate_voice_alert, display_alert_menu, voice_alerts



def cleanup(vid_writer):
...
def detect(opt, save_img=False):
    ...
    try:
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        # # Call the function to generate voice alerts
                        # generate_voice_alert(names[int(cls)])   
                        generate_voice_alert(int(opt.language_choice), names[int(cls)])  # generate_voice_alert(language_index - 1, sign_name)
                ...
                if view_img:
                    ...
                    if dataset.is_it_web:
                        if cv2.waitKey(1) & 0xFF == ord('q'):  # q to quit
                            cleanup(vid_writer)  # Clean up resources
                            break
                    else:
        ...
        pass  # Do nothing if KeyboardInterrupt occurs (expected when stopping detection)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--language-choice', type=int, default=0, help='language choice index (0 for English, 1 for Marathi, 2 for Hindi)')
